const searchtoilet = new Vue({
        ...
        axios
            ...
            .then((response) => {
                ...
                try {
                    ...
                    const member_part = value.split(`; member_id=`);
                    const admin_part = value.split(`; isAdmin=`);
                    if (member_part.length === 2) {
                        searchtoilet.member_id = member_part.pop().split(";").shift();
                        searchtoilet.isAdmin =
                            admin_part.pop().split(";").shift() === "true" ? true : false;
                    }
                } catch {}
            })
            .catch(() => {
    ...
    methods: {
        ...
        PositionToSearch: async () => {
            ...
            if (
                searchtoilet.selected.longitude == 0 ||
                searchtoilet.selected.latitude == 0
            ) {
                swal({
