const showtoilet = new Vue({
  ...
  created: () => {
    ...
    let requests = url.split("?");
    let district_id = 0;
    let toilet_id = 0;
    if (requests.length > 1) {
      let params = requests[1].split("&");
      for (let i = 0; i < params.length; i++) {
        let key_value = params[i].split("=");
        switch (key_value[0]) {
          case "district_id":
            district_id = key_value[1];
            break;
          case "toilet_id":
            toilet_id = key_value[1];
            break;
          default:
      ...
      if (checkIsInputedAndLegal) {
        axios
          ...
          .then(() => {
            ...
            //  showtoilet.review_info
            showtoilet.review_info[showtoilet.review_info.review_id - 1] =
              showtoilet.review_info;
          })
          .catch((error) => {
